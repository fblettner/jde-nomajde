...
public class jdebip {
 ...
 public static Blob  uncompress(Blob p_src,Connection conn) throws Exception {
   ...
   public static void main(String[] args) throws SQLException,IOException {
	...
	FileInputStream configStream = new FileInputStream("jdebip.properties");   
	try {
...
//    String password = Tools.decodePasswd(properties.getProperty("PASSWORD"));
    ...
    String sql = properties.getProperty(mode).toString();
    PreparedStatement stmt = conn.prepareStatement(sql);
	stmt.setInt(1,Integer.parseInt(args[4]));
    ...
    while( rs.next() ) {
    try {
    outBlob = uncompress(((OracleResultSet)rs).getBlob(column),conn);
        } catch ( Exception e ) {
            throw new SQLException(e);
        }
    }
    ...
    String remove_rd = properties.getProperty("REMOVE_RD");

    if (remove_rd.equals("Y")) {
        sql = properties.getProperty("DELETE_F9563110").toString();
        stmt = conn.prepareStatement(sql);
        stmt.setInt(1,Integer.parseInt(args[4]));
        stmt.executeUpdate();
        sql = properties.getProperty("DELETE_F95630").toString();
        stmt = conn.prepareStatement(sql);
        stmt.setInt(1,Integer.parseInt(args[4]));
        stmt.executeUpdate();   
    }
    ...
    try {
        String destination = properties.getProperty("OutputDirectory")+args[1]+"_"+args[2]+"_"+args[3]+"_"+args[4]+extension;
